...
void *clientHandler(void *args){
    thread_data *threadInfo=(thread_data *)args;
    while(chat(threadInfo->sockfd));
...
int main(){
    ...
    while(1){
        ...
        if(clientSocket<0){
            ...
            info->sockfd=clientSocket;
            pthread_t clientThread;
            int status=pthread_create(&clientThread,NULL,clientHandler,(void *)info);
            if(status!=0){
